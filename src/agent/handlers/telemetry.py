...
from reslib.schemas.telemetry import (
    ...
    MetricPayload as ReslibMetricPayload,
)
...
class RunTelemetry(BaseTelemetry):
    ...
    def emit_event(self, *, event: Union[ReslibEventPayload, EventPayload]) -> None:
        ...
        self.telemetry.emit_event(event=event, run_id=self.run_id)

    def emit_metrics(self, *, metrics: ReslibMetricPayload) -> None:
        """Forward metrics with the bound run and scenario IDs."""
        self.telemetry.emit_metrics(metrics=metrics, run_id=self.run_id)
